package com.mdcore.MysticalDiscoveries.world.biomes;

import java.util.Arrays;
import java.util.Random;

import com.mdcore.MysticalDiscoveries.world.biomes.biomegenerator.CustomBiomeTerrain;
...
public class BiomeGrassMesa extends CustomBiomeTerrain {
	...
	public BiomeGrassMesa() {
		super(new BiomeProperties("Grass Mesa").setBaseHeight(0.2F).setHeightVariation(0.35F).setRainfall(0.3F).setTemperature(1.4F));
		...
		usualTopBlock = this.topBlock;
		alternativeTopBlock = Blocks.DIRT.getDefaultState().withProperty(BlockDirt.VARIANT, BlockDirt.DirtType.COARSE_DIRT);
		
		fillerBlock = Blocks.DIRT.getDefaultState().withProperty(BlockDirt.VARIANT, BlockDirt.DirtType.COARSE_DIRT);
		normalFillerBlock = this.fillerBlock;
		scatteredFillerBlock = Blocks.HARDENED_CLAY.getDefaultState();
		
		underBlock = Blocks.HARDENED_CLAY.getDefaultState();
	...
	public void genTerrainBlocks(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal)
    {
        ...
        for (int j1 = 255; j1 >= 0; --j1)
        {
            ...
            {
                chunkPrimerIn.setBlockState(l1, j1, k1, HARDENED_CLAY);
            }
                ...
                else if (iblockstate1.getBlock() == Blocks.STONE)
                {
                    if (l == -1)
                    {
                        ...
                        else if (j1 >= i2 - 4 && j1 <= i2 + 1)
                        {
                        	...
                        	if (fillerType <= 4) {
                        		iblockstate3 = this.normalFillerBlock;
                        	} else {
                        		iblockstate3 = this.scatteredFillerBlock;
                        	}
                        }
                        ...
                        if (j1 >= i2 - 1)
                        {
                            if (j1 > i2 + 3 + k)
                            {
                                ...
                                if (j1 >= 64 && j1 <= 127)
                                {
                                    if (flag)
                                    {
                                    	...
                                    	if (topType <= 3) {
                                    		block = this.usualTopBlock;
                                    	} else {
                                    		block = this.alternativeTopBlock;
                                    	}
                                        iblockstate2 = block;
                                    }
                                    else
                            ...
                            else
                            {
                            	int topType = rand.nextInt(5);
                            	if (topType <= 3) {
                            		this.topBlock = this.usualTopBlock;
                            	} else {
                            		this.topBlock = this.alternativeTopBlock;
                            	}
    ...
    public void generateBands(long p_150619_1_)
    {
        ...
        for (int l1 = 0; l1 < 64; ++l1)
        {
            ...
            if (l1 < 64)
            {
                this.clayBands[l1] = HARDENED_CLAY;
            }
        }
